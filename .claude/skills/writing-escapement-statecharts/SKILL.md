...
- **Register the child chart in `:escapement/on-env-ready`, NOT `on-entry`** — `sp/register-statechart!` under the id `mo/child-params` resolves via `:src`. Put it in the chart var's `^{:escapement/on-env-ready (fn [env] …)}` metadata (a `(fn [env])` the runner runs on EVERY `run!` — fresh start AND resume — before start!/re-invoke; keep it idempotent). `on-entry` does NOT re-fire on resume (the state is already in the restored config), so an `on-entry`-registered child is missing on `--resume` / debugger branch re-run and the re-invoked multiplex silently fails to resolve it (dead branch `started → resumed → done`). `haiku_tournament_dynamic.clj` uses this. Guide.adoc "`:escapement/on-env-ready`".
