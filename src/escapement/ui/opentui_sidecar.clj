(ns escapement.ui.opentui-sidecar
...
(defn- repo-root
  "Walk up from cwd looking for an `opentui/` dir so the sidecar entry resolves
   regardless of where the CLI was launched."
  []
  (loop [d (.getAbsoluteFile (io/file (System/getProperty "user.dir")))]
    (cond
      (nil? d) nil
      (.isDirectory (io/file d "opentui")) d
      :else (recur (.getParentFile d)))))

(defn sidecar-entry
  "Resolve the absolute path of the Bun sidecar entry (`opentui/src/main.tsx`).
   Returns nil when no `opentui/` workspace is found."
  []
  (when-let [root (or (some-> (System/getenv "OPENTUI_DIR") io/file .getAbsoluteFile)
                      (some-> (repo-root) (io/file "opentui")))]
...
(defn spawn!
  "Launch the Bun sidecar. `opts`:
     * `:entry`       — absolute path to the sidecar entry (`opentui/src/main.tsx`).
  ...
  (let [bun     (find-bun)
        ...
        ;; Run with cwd = the opentui/ project dir (entry is opentui/src/main.tsx).
    ...
    ;; Export an ABSOLUTE session dir: the sidecar's cwd is opentui/ (set above),
