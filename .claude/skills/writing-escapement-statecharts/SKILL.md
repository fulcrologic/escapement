...
- **`:message` is a STRING** — it cannot carry an `:image` or any non-text block. For a multimodal or multi-message opening turn use `:initial-messages` (vector of pre-built message maps); it wins over `:message` and starts the worker in `:running`.
...
- **Ask for reasoning with `:reasoning`, never a provider's word** — `{:reasoning :high}` or `{:reasoning {:effort :high :budget-tokens N}}` (`:none :minimal :low :medium :high :max`). Each backend renders its own dialect; omitting it changes nothing on the wire; an explicit `:thinking` wins. On Anthropic-shaped backends it DROPS `:temperature`/`:top-p`/`:top-k` (warned once per process). Also accepted as a default on an `:llm/aliases` target.
- **Reasoning + continuation don't mix.** A reasoning model reads the prefilled partial turn as someone else's and restarts; the restart guard then fails the turn (`:error.llm.unexpected-stop` `:detail :continuation-restarted`) rather than stitch a duplicate. For a long artifact from a reasoning node, raise the output cap or set `:resilience {:continuation {:enabled? false}}` (one call, truncation is terminal — independent of `:overrun`). Continuation is otherwise bounded by `:max-segments 64` / `:max-chars 2000000`, and is skipped outright on endpoints declared unable to prefill (Ollama Cloud, `claude-cli`, the Responses wire).
- **A thinking model under a tight cap can return ONLY reasoning** (empty content, no error) — `run-turn` retries it once per `:max-retries` (`:on-retry` category `:reasoning-only`; DeepSeek/Ollama wires only). Prefer a bigger output cap over a bigger `:max-retries`.
