...
- [ ] For showing a pure function preserves its input: label says "preserves the original value" (not "does not modify
  the input")
- [ ] For showing a function targets a specific area: label says "focuses only on the target area" (not "doesn't modify
  other fields")
