(ns escapement.config
  "Loads `.escapement.edn` configuration. Two locations are consulted:

   1. `~/.escapement.edn` (user-global defaults)
   2. `./.escapement.edn` (project-local; overrides user-global)

   Returns a deep-merged map. Missing files are silently treated as `{}`,
