...
2. Spawn **separate fresh-context subagents**, one per concern, each given
   only the scope + the playbook + its single job. Do **not** perform any
   gate yourself:
   - Gate 1 (coverage & `bb test`/`bb sanity`, baseline attribution) —
     fresh subagent.
...
3. Spawn a **collator** subagent that transcribes each concern's returned
...
4. Report the OVERALL verdict (MERGEABLE / CONDITIONAL / BLOCKED) and, if
