(ns scale-test
  "Constrained-box scale test. Runs ONE arm at ONE concurrency C, streams a
  realistic mock LLM (TTFT + per-token delay), and reports peak RSS / threads /
  CPU / latency. Designed to be run inside `docker run --memory=4g --cpus=4`
  and ramped from the host until it OOMs (exit 137) or latency collapses.
...
;;   :turn  -> llm-conversation (one streamed turn) --:continue--> :tick
